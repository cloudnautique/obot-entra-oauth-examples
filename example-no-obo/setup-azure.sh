...
# Azure Entra ID app registration script for MCP Demo
...
echo ""
echo "Creating MCP Demo Server app..."
...
# Graph permission IDs (used by client app below)
...
echo ""
echo "Creating MCP Demo Client app..."
CLIENT_APP_ID=$(az ad app create \
    --display-name "MCP Demo Client" \
    --sign-in-audience AzureADMyOrg \
    --query appId -o tsv)
...
CLIENT_SECRET=$(az ad app credential reset \
    --id "$CLIENT_APP_ID" \
    --display-name "MCP Demo Secret" \
    --query password -o tsv)

# Add API permission for server app's access_as_user scope
az ad app permission add --id "$CLIENT_APP_ID" \
    --api "$SERVER_APP_ID" \
    --api-permissions "$SCOPE_ID=Scope"

# Add Microsoft Graph delegated permissions to client app
az ad app permission add --id "$CLIENT_APP_ID" \
    --api "$GRAPH_APP_ID" \
...
echo "====================================="
echo ""
echo "Server .env written with AZURE_TENANT_ID and BASE_URL."
echo ""
...
echo "  Scopes:            api://$SERVER_APP_ID/access_as_user User.Read Mail.Read offline_access"
echo ""
echo "── Redirect URI ─────────────────────────────────────────────────────────────"
echo ""
echo "Add your Obot callback URL as a redirect URI on the Client App:"
echo "  Azure Portal > App registrations > MCP Demo Client > Authentication > Add a platform > Web"
echo "  Example: http://localhost:8080/oauth/mcp/callback"
echo ""
echo "── Admin Consent ────────────────────────────────────────────────────────────"
echo ""
echo "Grant admin consent for the client app (Graph permissions) in Azure Portal:"
echo "  https://portal.azure.com/#view/Microsoft_AAD_RegisteredApps/ApplicationMenuBlade/~/CallAnAPI/appId/$CLIENT_APP_ID"
echo ""
